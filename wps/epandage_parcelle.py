...
class Process(WPSProcess):
    ...
    def __init__(self):
        ...
        WPSProcess.__init__(
            self,
            identifier="epandage_parcelle",
            title="Service WEB de calcul des SPE",
            version="1.0",
            storeSupported=True,
            statusSupported=True,
            grassLocation=True,
            abstract='''Service WPS pour le calcul des superficie des parcelles
            potentiellement epandables (SPE)''')
        ...
        self.parcelList = self.addLiteralInput(
            ...
            title="Identifiant(s) de(s) parcelle(s) - attributs 'parcelle_cdn' (separateur: virgule)",
            default="361667",
            type="",
            minOccurs=1)
...
#        LOGGER.info('\n%s\n' % (cmd))
        ...
        epandage_config_file = directories['epandage_parcelle_config_layers']
        ...
        grass.run_command(
            ...
            quiet=True,
            map='parcelle',
            columns='surf_parcelle_ha double precision')
        self.cmd(
            "v.to.db --quiet map=parcelle columns=surf_parcelle_ha option=area unit=hectares")
